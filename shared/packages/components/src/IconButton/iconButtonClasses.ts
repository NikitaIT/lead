...
export interface IconButtonClasses {
  ...
  textSuccess: string;
  /** Styles applied to the root element if `variant="text"` and `color="frontPage"`. */
  ...
  /** Styles applied to the root element if `variant="contained"` and `color="frontPage"`. */
  outlinedFrontPage: string;
...
export function getIconButtonUtilityClass(slot: string): string {
  return generateUtilityClass('MuiLeadIconButton', slot);
}

const iconButtonClasses = generateUtilityClasses<IconButtonClassKey>(
  'MuiLeadIconButton',
  [
    'root',
    'text',
